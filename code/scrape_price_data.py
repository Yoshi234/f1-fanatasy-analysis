...
def copilot_price_data_output():
    ...
    '''
    driversA = {
        "driver": ["PIA", "RUS", "LEC", "NOR", "HAM", "VER"],
        "price/$": [26.4, 22.4, 23.9, 31.4, 23.8, 28.2],
        "R13 Pts": [42, 19, 30, 39, 40, 30],
        "R14 Pts": [29, 37, 20, 36, 4, 11],
        "-0.3": [-24, -16, -7, -19, -1, 10],
        "-0.1": [-23, -15, -6, -18, 0, 11],
        "+0.1": [1, 5, 15, 10, 21, 36],
        "+0.3": [24, 25, 36, 38, 42, 61]
    }
    driversB = {
        "driver": ["BOR", "LAW", "ALO", "SAI", "STR", "HUL", "ALB", "OCO", "HAD", "COL", "ANT", "GAS", "BEA", "TSU"],
        "price/$": [4.5, 4.5, 7.5, 5.9, 9.7, 9.6, 11.2, 8.5, 5.1, 4.5, 15.9, 5.4, 7.5, 11.2],
        "R13 Pts": [7, 9, 7, 6, 9, 7, 14, 3, -7, -2, 21, -14, 7, 0],
        "R14 Pts": [24, 10, 17, 6, 13, 14, 12, 6, 4, -4, 10, 0, -18, 3],
        "-0.6": [np.nan, np.nan, -11, -2, -5, -4, -6, 6, 12, np.nan, -3, 23, 24, 17],
        "-0.2": [np.nan, np.nan, -10, -1, -4, -3, -5, 7, 13, np.nan, -2, 24, 25, 18],
        "0.0": [-19, -7, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, 18, np.nan, np.nan, np.nan, np.nan],
        "+0.2": [-18, -6, -3, 4, 5, 5, 5, 14, 17, 19, 12, 29, 32, 28],
        "+0.6": [-14, -2, 3, 10, 13, 14, 15, 22, 22, 23, 26, 34, 38, 38]
    }
    constructorsA = {
        "constructor": ["MCL", "MER", "FER", "RED"],
        "price/$": [34.2, 25.7, 30.3, 28.4],
        "R13 Pts": [93, 50, 70, 55],
    ...
    constructorsB = {
        "constructor": ["KCK", "AST", "VRB", "WIL", "HAA", "ALP"],
        "price/$": [9.3, 11.3, 11.2, 15.5, 11.8, 8.3],
        "R13 Pts": [24, 20, 17, 27, 15, -13],
        "R14 Pts": [35, 45, 29, 24, -9, 2],
        "-0.6": [-43, -45, -26, -23, 15, 26],
        "-0.2": [-42, -44, -25, -22, 16, 27],
        "+0.2": [-33, -34, -15, -9, 26, 34],
        "+0.6": [-25, -24, -5, 5, 37, 41]
    }
